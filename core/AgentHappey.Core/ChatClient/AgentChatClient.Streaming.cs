...
public partial class AgentChatClient
{
    ...
    private static Dictionary<string, Dictionary<string, object>>? BuildReasoningProviderMetadata(
        ResponseStreamItem item,
        string? providerId)
    {
        var providerMetadata = new Dictionary<string, Dictionary<string, object>>(StringComparer.Ordinal);

        if (item.AdditionalProperties?.TryGetValue("provider_metadata", out var rawProviderMetadata) == true
            && rawProviderMetadata.ValueKind == JsonValueKind.Object)
        {
            foreach (var providerEntry in rawProviderMetadata.EnumerateObject())
            {
                if (providerEntry.Value.ValueKind != JsonValueKind.Object)
                    continue;

                var values = providerEntry.Value.EnumerateObject()
                ...
                if (values.Count > 0)
                    providerMetadata[providerEntry.Name] = values;
            }
        }

        var resolvedProviderId = providerId;
        if (string.IsNullOrWhiteSpace(resolvedProviderId) && providerMetadata.Count == 1)
            resolvedProviderId = providerMetadata.Keys.Single();

        if (!string.IsNullOrWhiteSpace(resolvedProviderId))
        {
            if (!providerMetadata.TryGetValue(resolvedProviderId!, out var values))
            {
                values = new Dictionary<string, object>(StringComparer.Ordinal);
                providerMetadata[resolvedProviderId!] = values;
            }
            ...
            if (item.AdditionalProperties?.TryGetValue("encrypted_content", out var encryptedContent) == true
                && HasMeaningfulReasoningValue(encryptedContent))
            {
                var value = ToPlainObject(encryptedContent);
                if (value is not null)
                    values["encrypted_content"] = value;
            }

            if (item.AdditionalProperties?.TryGetValue("summary", out var summary) == true
                && HasMeaningfulReasoningValue(summary))
            {
                var value = ToPlainObject(summary);
                if (value is not null)
    ...
    private static bool HasMeaningfulReasoningValue(JsonElement value)
